...
function terraformDestroy {
  ...
  echo "destroy: info: destroying Terraform environment in ${tfWorkingDir}"
  ...
  if [ ${destroyExitCode} -eq 0 ]; then
    echo "destroy: info: successfully destroyed Terraform environment in ${tfWorkingDir}"
    echo "${destroyOutput}"
    echo
  ...
  if [ ${destroyExitCode} -ne 0 ]; then
    echo "destroy: error: failed to destroy Terraform environment in ${tfWorkingDir}"
    echo "${destroyOutput}"
    echo
  fi
